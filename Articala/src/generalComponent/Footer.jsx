import React from 'react'
import Logo from './Logo'
import { Col, Container, Row } from 'react-bootstrap'
import AppLink from './AppLink'


const Footer = () => {
    return (
        <>
            <div className="footerBox py-5">

                <Container>
                    <Row>
                        <Col xl={3} sm={6}>
                            <div className="d-flex flex-column gap-3 pb-4">
                                <Logo />
                                <p className='BodyMedium400 textGray500'>
                                    Aliquam rhoncus ligula est, non pulvinar elit
                                    convallis nec. Donec mattis odio at.
                                </p>
                                <div className="d-xl-flex d-none gap-3">
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M11.3575 2.98875H13.0008V0.12675C12.7172 0.08775 11.7423 0 10.6068 0C8.2375 0 6.6145 1.49025 6.6145 4.22925V6.75H4V9.9495H6.6145V18H9.82V9.95025H12.3288L12.727 6.75075H9.81925V4.5465C9.82 3.62175 10.069 2.98875 11.3575 2.98875Z" fill="white" />
                                            </svg>
                                        </a>
                                    </div>
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M17.9507 5.29205C17.9086 4.33564 17.7539 3.67812 17.5324 3.10836C17.3038 2.50359 16.9522 1.96213 16.4915 1.51201C16.0414 1.05489 15.4963 0.699691 14.8986 0.474702C14.3255 0.253147 13.6714 0.0984842 12.715 0.0563159C11.7515 0.0105764 11.4456 0 9.00174 0C6.55791 0 6.25202 0.0105764 5.29204 0.0527447C4.33563 0.0949129 3.67811 0.249713 3.1085 0.471131C2.50358 0.699691 1.96213 1.05132 1.51201 1.51201C1.05489 1.96213 0.699827 2.50716 0.474701 3.10493C0.253147 3.67812 0.098484 4.33207 0.0563158 5.28848C0.0105764 6.25203 0 6.55792 0 9.00176C0 11.4456 0.0105764 11.7515 0.0527446 12.7115C0.0949128 13.6679 0.249713 14.3254 0.471267 14.8952C0.699827 15.4999 1.05489 16.0414 1.51201 16.4915C1.96213 16.9486 2.50715 17.3038 3.10493 17.5288C3.67811 17.7504 4.33206 17.905 5.28861 17.9472C6.24845 17.9895 6.55448 17.9999 8.99831 17.9999C11.4421 17.9999 11.748 17.9895 12.708 17.9472C13.6644 17.905 14.3219 17.7504 14.8916 17.5288C16.1012 17.0611 17.0577 16.1047 17.5254 14.8952C17.7468 14.322 17.9016 13.6679 17.9437 12.7115C17.9859 11.7515 17.9965 11.4456 17.9965 9.00176C17.9965 6.55792 17.9929 6.25203 17.9507 5.29205ZM16.3298 12.6411C16.2911 13.5202 16.1434 13.9949 16.0203 14.3114C15.7179 15.0956 15.0955 15.7179 14.3114 16.0204C13.9949 16.1434 13.5168 16.2911 12.6411 16.3297C11.6917 16.372 11.407 16.3824 9.00531 16.3824C6.60365 16.3824 6.31534 16.372 5.36937 16.3297C4.4903 16.2911 4.01559 16.1434 3.69913 16.0204C3.3089 15.8761 2.9537 15.6476 2.66539 15.3487C2.3665 15.0568 2.13794 14.7052 1.99372 14.315C1.87065 13.9985 1.72299 13.5202 1.68439 12.6447C1.64209 11.6953 1.63165 11.4104 1.63165 9.00876C1.63165 6.60709 1.64209 6.31878 1.68439 5.37295C1.72299 4.49387 1.87065 4.01917 1.99372 3.7027C2.13794 3.31234 2.3665 2.95727 2.66896 2.66883C2.9607 2.36994 3.31233 2.14138 3.7027 1.99729C4.01917 1.87422 4.49744 1.72656 5.37294 1.68783C6.32235 1.64566 6.60722 1.63508 9.00875 1.63508C11.414 1.63508 11.6987 1.64566 12.6447 1.68783C13.5238 1.72656 13.9985 1.87422 14.3149 1.99729C14.7052 2.14138 15.0604 2.36994 15.3487 2.66883C15.6476 2.96071 15.8761 3.31234 16.0203 3.7027C16.1434 4.01917 16.2911 4.49731 16.3298 5.37295C16.372 6.32236 16.3826 6.60709 16.3826 9.00876C16.3826 11.4104 16.372 11.6917 16.3298 12.6411Z" fill="white" />
                                                <path d="M9.00188 4.37695C6.44912 4.37695 4.37793 6.44801 4.37793 9.0009C4.37793 11.5538 6.44912 13.6249 9.00188 13.6249C11.5548 13.6249 13.6258 11.5538 13.6258 9.0009C13.6258 6.44801 11.5548 4.37695 9.00188 4.37695ZM9.00188 12.0003C7.34578 12.0003 6.00244 10.6571 6.00244 9.0009C6.00244 7.34467 7.34578 6.00146 9.00188 6.00146C10.6581 6.00146 12.0013 7.34467 12.0013 9.0009C12.0013 10.6571 10.6581 12.0003 9.00188 12.0003Z" fill="white" />
                                                <path d="M14.8876 4.19472C14.8876 4.79085 14.4043 5.2742 13.808 5.2742C13.2119 5.2742 12.7285 4.79085 12.7285 4.19472C12.7285 3.59845 13.2119 3.11523 13.808 3.11523C14.4043 3.11523 14.8876 3.59845 14.8876 4.19472Z" fill="white" />
                                            </svg>

                                        </a>
                                    </div>
                                    <div className="socialLinkBox glow">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M17.9955 18.0002V17.9994H18V11.3979C18 8.16841 17.3047 5.68066 13.5292 5.68066C11.7142 5.68066 10.4962 6.67666 9.99896 7.62091H9.94646V5.98216H6.3667V17.9994H10.0942V12.0489C10.0942 10.4822 10.3912 8.96716 12.3315 8.96716C14.2432 8.96716 14.2717 10.7552 14.2717 12.1494V18.0002H17.9955Z" fill="#7E54E0" />
                                                <path d="M0.296997 5.98242H4.029V17.9997H0.296997V5.98242Z" fill="#7E54E0" />
                                                <path d="M2.1615 0C0.96825 0 0 0.96825 0 2.1615C0 3.35475 0.96825 4.34325 2.1615 4.34325C3.35475 4.34325 4.323 3.35475 4.323 2.1615C4.32225 0.96825 3.354 0 2.1615 0V0Z" fill="#7E54E0" />
                                            </svg>

                                        </a>
                                    </div>
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g clipPath="url(#clip0_762_513)">
                                                    <path d="M18 3.73137C17.3306 4.025 16.6174 4.21962 15.8737 4.31412C16.6388 3.85737 17.2226 3.13962 17.4971 2.2745C16.7839 2.69975 15.9964 3.00013 15.1571 3.16775C14.4799 2.44663 13.5146 2 12.4616 2C10.4186 2 8.77387 3.65825 8.77387 5.69113C8.77387 5.98363 8.79862 6.26487 8.85938 6.53262C5.7915 6.383 3.07687 4.91263 1.25325 2.67275C0.934875 3.22513 0.748125 3.85738 0.748125 4.538C0.748125 5.816 1.40625 6.94887 2.38725 7.60475C1.79437 7.5935 1.21275 7.42138 0.72 7.15025C0.72 7.1615 0.72 7.17613 0.72 7.19075C0.72 8.984 1.99912 10.4735 3.6765 10.8166C3.37612 10.8988 3.04875 10.9381 2.709 10.9381C2.47275 10.9381 2.23425 10.9246 2.01038 10.8751C2.4885 12.3365 3.84525 13.4109 5.4585 13.4457C4.203 14.4279 2.60888 15.0196 0.883125 15.0196C0.5805 15.0196 0.29025 15.0061 0 14.969C1.63462 16.0231 3.57188 16.625 5.661 16.625C12.4515 16.625 16.164 11 16.164 6.12425C16.164 5.96112 16.1584 5.80363 16.1505 5.64725C16.8829 5.1275 17.4982 4.47837 18 3.73137Z" fill="white" />
                                                </g>
                                                <defs>
                                                    <clipPath id="clip0_762_513">
                                                        <rect width="18" height="18" fill="white" />
                                                    </clipPath>
                                                </defs>
                                            </svg>

                                        </a>
                                    </div>
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M9 3C9.03603 3 14.6389 3.0015 16.043 3.38574C16.8137 3.59342 17.4213 4.20087 17.6289 4.97168C18.0145 6.38012 18 9.31641 18 9.31641C18 9.33769 17.9982 12.2421 17.6289 13.6455C17.4213 14.4164 16.814 15.0249 16.043 15.2324C14.6365 15.6027 9.01668 15.6025 9 15.6025C9 15.6025 3.3803 15.6033 1.95703 15.2178C1.186 15.0101 0.578752 14.4018 0.371094 13.6309C0.00261305 12.2453 1.26186e-05 9.33576 0 9.30176C0 9.30176 0.000444037 6.38012 0.371094 4.97168C0.578707 4.20096 1.2011 3.57859 1.95703 3.37109C3.36561 3.00031 9 3 9 3ZM7.20605 12L11.8916 9.30176L7.20605 6.60352V12Z" fill="white" />
                                            </svg>

                                        </a>
                                    </div>
                                </div>
                            </div>
                        </Col>

                        <Col xl={6} lg={12} className='order-2 order-xl-1'>
                            <div className="h-100 pt-5 pb-4">

                                <Row className='h-100'>
                                    <Col md={4} xl={4} lg={3}>
                                        <div className="d-flex flex-column h-100 justify-content-center pb-4">
                                            <h4 className='textWhite labalLarge'>Top 4 Category</h4>
                                            <ul className='d-flex flex-column'>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Development
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Finance & Accounting
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Design
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Business
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
                                    </Col>
                                    <Col md={4} xl={4}  lg={3}>
                                        <div className="d-flex flex-column h-100 justify-content-center pb-4">
                                            <h4 className='textWhite labalLarge'>Top 4 Category</h4>
                                            <ul className='d-flex flex-column'>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Development
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Finance & Accounting
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Design
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Business
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
                                    </Col>
                                    <Col md={4} xl={4} lg={3}>
                                        <div className="d-flex flex-column h-100 justify-content-center pb-4">
                                            <h4 className='textWhite labalLarge'>Top 4 Category</h4>
                                            <ul className='d-flex flex-column'>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Development
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Finance & Accounting
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Design
                                                    </a>
                                                </li>
                                                <li>
                                                    <a className='BodyMedium400 textGray500' href="">
                                                        Business
                                                    </a>
                                                </li>
                                            </ul>
                                        </div>
                                    </Col>
                                    <Col lg={3} className='d-xl-none '>
                                        <div className="d-flex w-100  flex-wrap align-content-center justify-content-lg-end justify-content-between gap-3">
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M11.3575 2.98875H13.0008V0.12675C12.7172 0.08775 11.7423 0 10.6068 0C8.2375 0 6.6145 1.49025 6.6145 4.22925V6.75H4V9.9495H6.6145V18H9.82V9.95025H12.3288L12.727 6.75075H9.81925V4.5465C9.82 3.62175 10.069 2.98875 11.3575 2.98875Z" fill="white" />
                                            </svg>
                                        </a>
                                    </div>
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M17.9507 5.29205C17.9086 4.33564 17.7539 3.67812 17.5324 3.10836C17.3038 2.50359 16.9522 1.96213 16.4915 1.51201C16.0414 1.05489 15.4963 0.699691 14.8986 0.474702C14.3255 0.253147 13.6714 0.0984842 12.715 0.0563159C11.7515 0.0105764 11.4456 0 9.00174 0C6.55791 0 6.25202 0.0105764 5.29204 0.0527447C4.33563 0.0949129 3.67811 0.249713 3.1085 0.471131C2.50358 0.699691 1.96213 1.05132 1.51201 1.51201C1.05489 1.96213 0.699827 2.50716 0.474701 3.10493C0.253147 3.67812 0.098484 4.33207 0.0563158 5.28848C0.0105764 6.25203 0 6.55792 0 9.00176C0 11.4456 0.0105764 11.7515 0.0527446 12.7115C0.0949128 13.6679 0.249713 14.3254 0.471267 14.8952C0.699827 15.4999 1.05489 16.0414 1.51201 16.4915C1.96213 16.9486 2.50715 17.3038 3.10493 17.5288C3.67811 17.7504 4.33206 17.905 5.28861 17.9472C6.24845 17.9895 6.55448 17.9999 8.99831 17.9999C11.4421 17.9999 11.748 17.9895 12.708 17.9472C13.6644 17.905 14.3219 17.7504 14.8916 17.5288C16.1012 17.0611 17.0577 16.1047 17.5254 14.8952C17.7468 14.322 17.9016 13.6679 17.9437 12.7115C17.9859 11.7515 17.9965 11.4456 17.9965 9.00176C17.9965 6.55792 17.9929 6.25203 17.9507 5.29205ZM16.3298 12.6411C16.2911 13.5202 16.1434 13.9949 16.0203 14.3114C15.7179 15.0956 15.0955 15.7179 14.3114 16.0204C13.9949 16.1434 13.5168 16.2911 12.6411 16.3297C11.6917 16.372 11.407 16.3824 9.00531 16.3824C6.60365 16.3824 6.31534 16.372 5.36937 16.3297C4.4903 16.2911 4.01559 16.1434 3.69913 16.0204C3.3089 15.8761 2.9537 15.6476 2.66539 15.3487C2.3665 15.0568 2.13794 14.7052 1.99372 14.315C1.87065 13.9985 1.72299 13.5202 1.68439 12.6447C1.64209 11.6953 1.63165 11.4104 1.63165 9.00876C1.63165 6.60709 1.64209 6.31878 1.68439 5.37295C1.72299 4.49387 1.87065 4.01917 1.99372 3.7027C2.13794 3.31234 2.3665 2.95727 2.66896 2.66883C2.9607 2.36994 3.31233 2.14138 3.7027 1.99729C4.01917 1.87422 4.49744 1.72656 5.37294 1.68783C6.32235 1.64566 6.60722 1.63508 9.00875 1.63508C11.414 1.63508 11.6987 1.64566 12.6447 1.68783C13.5238 1.72656 13.9985 1.87422 14.3149 1.99729C14.7052 2.14138 15.0604 2.36994 15.3487 2.66883C15.6476 2.96071 15.8761 3.31234 16.0203 3.7027C16.1434 4.01917 16.2911 4.49731 16.3298 5.37295C16.372 6.32236 16.3826 6.60709 16.3826 9.00876C16.3826 11.4104 16.372 11.6917 16.3298 12.6411Z" fill="white" />
                                                <path d="M9.00188 4.37695C6.44912 4.37695 4.37793 6.44801 4.37793 9.0009C4.37793 11.5538 6.44912 13.6249 9.00188 13.6249C11.5548 13.6249 13.6258 11.5538 13.6258 9.0009C13.6258 6.44801 11.5548 4.37695 9.00188 4.37695ZM9.00188 12.0003C7.34578 12.0003 6.00244 10.6571 6.00244 9.0009C6.00244 7.34467 7.34578 6.00146 9.00188 6.00146C10.6581 6.00146 12.0013 7.34467 12.0013 9.0009C12.0013 10.6571 10.6581 12.0003 9.00188 12.0003Z" fill="white" />
                                                <path d="M14.8876 4.19472C14.8876 4.79085 14.4043 5.2742 13.808 5.2742C13.2119 5.2742 12.7285 4.79085 12.7285 4.19472C12.7285 3.59845 13.2119 3.11523 13.808 3.11523C14.4043 3.11523 14.8876 3.59845 14.8876 4.19472Z" fill="white" />
                                            </svg>

                                        </a>
                                    </div>
                                    <div className="socialLinkBox glow">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M17.9955 18.0002V17.9994H18V11.3979C18 8.16841 17.3047 5.68066 13.5292 5.68066C11.7142 5.68066 10.4962 6.67666 9.99896 7.62091H9.94646V5.98216H6.3667V17.9994H10.0942V12.0489C10.0942 10.4822 10.3912 8.96716 12.3315 8.96716C14.2432 8.96716 14.2717 10.7552 14.2717 12.1494V18.0002H17.9955Z" fill="#7E54E0" />
                                                <path d="M0.296997 5.98242H4.029V17.9997H0.296997V5.98242Z" fill="#7E54E0" />
                                                <path d="M2.1615 0C0.96825 0 0 0.96825 0 2.1615C0 3.35475 0.96825 4.34325 2.1615 4.34325C3.35475 4.34325 4.323 3.35475 4.323 2.1615C4.32225 0.96825 3.354 0 2.1615 0V0Z" fill="#7E54E0" />
                                            </svg>

                                        </a>
                                    </div>
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <g clipPath="url(#clip0_762_513)">
                                                    <path d="M18 3.73137C17.3306 4.025 16.6174 4.21962 15.8737 4.31412C16.6388 3.85737 17.2226 3.13962 17.4971 2.2745C16.7839 2.69975 15.9964 3.00013 15.1571 3.16775C14.4799 2.44663 13.5146 2 12.4616 2C10.4186 2 8.77387 3.65825 8.77387 5.69113C8.77387 5.98363 8.79862 6.26487 8.85938 6.53262C5.7915 6.383 3.07687 4.91263 1.25325 2.67275C0.934875 3.22513 0.748125 3.85738 0.748125 4.538C0.748125 5.816 1.40625 6.94887 2.38725 7.60475C1.79437 7.5935 1.21275 7.42138 0.72 7.15025C0.72 7.1615 0.72 7.17613 0.72 7.19075C0.72 8.984 1.99912 10.4735 3.6765 10.8166C3.37612 10.8988 3.04875 10.9381 2.709 10.9381C2.47275 10.9381 2.23425 10.9246 2.01038 10.8751C2.4885 12.3365 3.84525 13.4109 5.4585 13.4457C4.203 14.4279 2.60888 15.0196 0.883125 15.0196C0.5805 15.0196 0.29025 15.0061 0 14.969C1.63462 16.0231 3.57188 16.625 5.661 16.625C12.4515 16.625 16.164 11 16.164 6.12425C16.164 5.96112 16.1584 5.80363 16.1505 5.64725C16.8829 5.1275 17.4982 4.47837 18 3.73137Z" fill="white" />
                                                </g>
                                                <defs>
                                                    <clipPath id="clip0_762_513">
                                                        <rect width="18" height="18" fill="white" />
                                                    </clipPath>
                                                </defs>
                                            </svg>

                                        </a>
                                    </div>
                                    <div className="socialLinkBox">
                                        <a className='BodyMedium400 textGray500' href="">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M9 3C9.03603 3 14.6389 3.0015 16.043 3.38574C16.8137 3.59342 17.4213 4.20087 17.6289 4.97168C18.0145 6.38012 18 9.31641 18 9.31641C18 9.33769 17.9982 12.2421 17.6289 13.6455C17.4213 14.4164 16.814 15.0249 16.043 15.2324C14.6365 15.6027 9.01668 15.6025 9 15.6025C9 15.6025 3.3803 15.6033 1.95703 15.2178C1.186 15.0101 0.578752 14.4018 0.371094 13.6309C0.00261305 12.2453 1.26186e-05 9.33576 0 9.30176C0 9.30176 0.000444037 6.38012 0.371094 4.97168C0.578707 4.20096 1.2011 3.57859 1.95703 3.37109C3.36561 3.00031 9 3 9 3ZM7.20605 12L11.8916 9.30176L7.20605 6.60352V12Z" fill="white" />
                                            </svg>

                                        </a>
                                    </div>
                                </div>
                                    </Col>
                                </Row>
                            </div>
                        </Col>
                        <Col xl={3} sm={6} className='order-xl-2'>
                            <div className="d-flex flex-sm-column flex-row gap-3 align-items-sm-end align-items-start">
                                <div className="d-flex flex-column gap-3">

                                <h4 className='labalLarge textWhite'>
                                    Downlaod our app
                                </h4>
                                <div className="d-flex flex-sm-column flex-row gap-3">

                                <AppLink isAppStore={1} />
                                <AppLink isAppStore={0} />
                                </div>
                                </div>
                            </div>
                        </Col>
                    </Row>
                </Container>
            </div>
            <div className="footerBottom">
                <Container>
                    <div
                    className='d-flex justify-content-center py-2'
                    >

                    <p>
                        © 2025 - All rights reserved
                    </p>
                    </div>
                </Container>
            </div>
        </>
    )
}

export default Footer